    ...
    def handle(self, added, removed):
        ...
        for a in added:
            ...
            if not thing in self.thingsDict:
                ...
                logging.debug(self.baseURL + '/things/' + self.thingsDict[thing]["id"])
                logging.debug(self.baseURL + '/things/' + self.thingsDict[thing]["id"] + "/events")
                logging.debug(self.baseURL + '/things/' + self.thingsDict[thing]["id"] + "/actions")
                logging.debug(self.baseURL + '/things/' + self.thingsDict[thing]["id"] + "/properties")
                self.flaskApp.add_url_rule('/things/' + self.thingsDict[thing]["id"], "td", self.invFunct)
                self.flaskApp.add_url_rule('/things/' + self.thingsDict[thing]["id"] + "/events", "events", self.invFunct)
                self.flaskApp.add_url_rule('/things/' + self.thingsDict[thing]["id"] + "/actions", "actions", self.invFunct)
                self.flaskApp.add_url_rule('/things/' + self.thingsDict[thing]["id"] + "/properties", "properties", self.invFunct)
